...
export class UIDateView {
  ...
  private refresh() {
    if (this.date && moment(this.date).isValid()) {
      ...
      else if (this.maxDate && moment(this.date).isAfter(this.maxDate, 'date')) this.date = this.maxDate;
...
export class UIDateInput extends UIBaseInput {
  ...
  private keyDown(evt) {
    ...
    if (code === 38) {
      if (this.date && this.maxDate && moment(this.date).add(1, 'day').isAfter(this.maxDate, 'date')) return;
      else if (this.date && moment(this.date).isValid()) this.date = moment(this.date).add(1, 'day').toISOString();
      else this.date = moment().toISOString();
    }
    if (code === 40) {
      if (this.date && this.minDate && moment(this.date).add(-1, 'day').isBefore(this.minDate, 'date')) return;
      else if (this.date && moment(this.date).isValid()) this.date = moment(this.date).add(-1, 'day').toISOString();
      else this.date = moment().toISOString();
    }
    if (code === 37) { //left
      if (this.date && this.minDate && moment(this.date).add(-1, 'month').isBefore(this.minDate, 'date')) return;
      else if (this.date && moment(this.date).isValid()) this.date = moment(this.date).add(-1, 'month').toISOString();
      else this.date = moment().toISOString();
    }
    if (code === 39) { //right
      if (this.date && this.maxDate && moment(this.date).add(1, 'month').isAfter(this.maxDate, 'date')) return;
      else if (this.date && moment(this.date).isValid()) this.date = moment(this.date).add(1, 'month').toISOString();
      else this.date = moment().toISOString();
    }
  }
